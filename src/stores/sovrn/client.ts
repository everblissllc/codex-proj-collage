...
export class SovrnClient {
  ...
  async approvedMerchants(domains: readonly string[], requestId?: string): Promise<unknown> {
    return (await this.approvedMerchantsDetailed(domains, requestId)).value;
  }

  async approvedMerchantsDetailed(domains: readonly string[], requestId?: string): Promise<SovrnApiResult> {
    if (!this.config.campaignId || !/^\d+$/.test(this.config.campaignId)) throw new ProductError("SOVRN_CONFIG_MISSING", "configuration", "Sovrn campaign ID is required");
    const endpoint = new URL("https://viglink.io/merchants/rates/summaries");
    endpoint.searchParams.set("campaignId", this.config.campaignId);
    const response = await this.fetcher(endpoint.href, {
      method: "POST", signal: AbortSignal.timeout(12_000),
      headers: { accept: "application/json", authorization: `secret ${this.config.secretKey}`, "content-type": "application/json" },
      body: JSON.stringify({ filters: [{ type: "DOMAIN", values: domains }], page: 1, pageSize: Math.max(20, domains.length) })
    });
    if (response.status === 429) { await response.body?.cancel(); throw new SovrnApiError("SOVRN_RATE_LIMITED", "Approved Merchants rate limit exceeded", 429); }
    if (response.status === 401 || response.status === 403) { await response.body?.cancel(); throw new SovrnApiError("SOVRN_AUTH_FAILED", "Approved Merchants authorization failed", response.status); }
    if (!response.ok) { await response.body?.cancel(); throw new SovrnApiError("SOVRN_API_ERROR", `Approved Merchants returned HTTP ${response.status}`, response.status); }
    console.log(JSON.stringify({ event: "sovrn_request_complete", requestId, operation: "approved_merchants", httpStatus: response.status }));
    return { httpStatus: response.status, value: await safeJson(response) };
  }
}
