...
export function walmartCardHtml(product: ProductData, content: GeneratedContent, imageDataUrl: string): string {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src data:; style-src 'unsafe-inline'"><meta name="viewport" content="width=${t.width}, initial-scale=1"><style>
  ...
  </style></head><body><main class="card"><div class="image-area"><img src="${imageDataUrl}" alt=""></div><div class="title-area"><div class="title">${title}</div></div><div class="price-row"><span class="current-price"><span class="now">Now</span>${current}</span>${old}</div></main></body></html>`;
}
