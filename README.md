...
For Amazon, the Worker first reads an allowlisted Amazon PDP response. If the response is a challenge or lacks a complete primary product offer, it uses Browser Run's ordinary `content` action as a fallback and revalidates the final Amazon hostname. Extraction binds the resolved ASIN to the page's hidden/canonical ASIN, prefers a matching JSON-LD new purchasable offer, and falls back to the primary Buy Box price region and product metadata. Explicit struck/list prices are accepted only when greater than the current one-time price. Subscribe & Save, installment, unit, coupon, business, used, and alternate-marketplace amounts do not become the normal current price. The image is embedded into a distinct 1200×1200 Amazon deal-card template; Amazon never uses the screenshot-store path.
